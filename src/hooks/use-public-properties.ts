...
import { properties as demoProperties, type Property, type PropertyType, type Listing } from "@/data/properties";
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<Property[]> => {
      const { data, error } = await supabase
        .from("properties")
        .select("id,title,type,status,price,city,district,area,floor,description,main_image,is_featured,created_at")
